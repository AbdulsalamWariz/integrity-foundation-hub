...
const queryBySlug = `*[_type == "project" && slug.current == $slug][0]{
  _id,
  title,
  publishedAt,
        ...
        <div className="min-h-screen bg-background text-foreground w-full overflow-x-hidden">
            ...
            <main className="px-3 sm:px-4 py-6 sm:py-12 w-full">
                <div className="max-w-3xl mx-auto w-full mb-4 sm:mb-6 px-0">
                    <Link to="/projects" className="text-xs sm:text-sm text-muted-foreground hover:underline inline-block break-words">← Back to Projects</Link>
                </div>

                {isLoading && <p className="text-muted-foreground">Loading…</p>}
                {isError && <p className="text-destructive">Failed to load project.</p>}
                    ...
                    <div className="max-w-3xl mx-auto w-full px-0">
                        <Card className="border-none shadow-lg w-full">
                            <CardContent className="p-4 sm:p-6 w-full overflow-hidden">
                                ...
                                {data.mainImage && (
                                    <img src={data.mainImage} alt={data.title} className="mt-4 sm:mt-6 w-full h-auto rounded-md object-cover" />
                                )}
